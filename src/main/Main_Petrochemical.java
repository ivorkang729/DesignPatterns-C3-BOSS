...
public class Main_Petrochemical {
	...
	public void main() throws URISyntaxException {
		...
		TestdataOutReader.init(testfilePath + "petrochemical.out");
		TestdataInReader testdataInReader = new TestdataInReader(testfilePath + "petrochemical.in");
		DecisionMaker testDataDecisionMaker = new TestDataDecisionMaker(testdataInReader);
		
//		Troop1
		Troop troop1 = new Troop();
		
		Unit unit = new Unit("英雄", 400, 99999, 30, testDataDecisionMaker);
...
//		Troop2
		...
		unit = new Unit("攻擊力超強的BOSS", 270, 9999, 399, new SimpleAiDecisionMaker());
		unit.addSkill(new Petrochemical());
		troop2.addUnit(unit);
